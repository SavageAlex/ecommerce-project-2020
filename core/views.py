        ...
        try:
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    ...
                    shippung_address1 = form.cleaned_data.get(
                        'shippung_address')
                    ...
                    if is_valid_form([shippung_address1, shipping_country, shipping_zip]):
                        shipping_address = Address(
                            user=self.request.user,
                            street_address=shippung_address1,
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            if not OrderItem.objects.filter(user=request.user, ordered=False).exists():
                order_qs.delete()
                messages.info(request, "You order is empty.")
